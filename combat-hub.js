...
// v6.5.0-github — visual consistency pass / no Vercel dependency

(async()=>{
const VERSION='6.5.0-github';
...
const VISUAL={
 ...
 k1:{posterShade:.88,heroShade:.80,topShade:.12,bottomShade:.24,accentVeil:.025,heroGap:18,mainSize:11.9,divisionSize:7.2}
};
const V=VISUAL[KEY];
...
function posterBg(image){const c=new DrawContext();c.size=new Size(720,338);c.opaque=true;c.respectScreenScale=false;c.setFillColor(new Color('#050609'));c.fillRect(new Rect(0,0,720,338));if(image?.size){const iw=image.size.width,ih=image.size.height,scale=Math.max(720/iw,338/ih),dw=iw*scale,dh=ih*scale;c.drawImageInRect(image,new Rect((720-dw)/2,(338-dh)/2,dw,dh));}c.setFillColor(new Color('#000000',V.posterShade));c.fillRect(new Rect(0,0,720,338));c.setFillColor(new Color('#000000',V.topShade));c.fillRect(new Rect(0,0,720,118));c.setFillColor(new Color('#000000',V.bottomShade));c.fillRect(new Rect(0,150,720,188));c.setFillColor(new Color(S.accent,V.accentVeil));c.fillRect(new Rect(0,0,720,338));return c.getImage();}
