...
// v6.3.0-github — no Vercel dependency

(async () => {
const VERSION='6.3.0-github';
...
const SNAPSHOT={
  ...
  boxing:{startAt:'2026-09-12T12:00:00-07:00',location:'Las Vegas',timeTba:true,main:{a:'RYAN GARCIA',b:'CONOR BENN',context:'WBC WELTERWEIGHT WORLD TITLE'},support:[],source:'https://www.ringmagazine.com/events'},
  k1:{startAt:'2026-09-12T12:00:00+09:00',location:'東京',main:{a:'AKIHIRO KANEKO',b:'RIAMU',context:'SUPER BANTAMWEIGHT TITLE'},support:[{label:'TITLE FIGHT',a:'朝久泰央',b:'アラッサン・カマラ'},{label:'TITLE FIGHT',a:'里見柚己',b:'永澤サムエル聖光'}],source:'https://www.k-1.co.jp/k-1wgp/schedule'}
};
...
function tx(st,s,z,c,w='regular',n=1){const t=st.addText(String(s??''));t.font=fnt(z,w);t.textColor=c;t.lineLimit=n;t.minimumScaleFactor=.45;return t;}
...
const RIZIN_PROFILES={
  'ラジャブアリ・シェイドゥラエフ':'https://jp.rizinff.com/_tags/%E3%83%A9%E3%82%B8%E3%83%A3%E3%83%96%E3%82%A2%E3%83%AA%E3%83%BB%E3%82%B7%E3%82%A7%E3%82%A4%E3%83%89%E3%82%A5%E3%83%A9%E3%82%A8%E3%83%95',
  'AJ・マッキー':'https://jp.rizinff.com/_tags/AJ%E3%83%BB%E3%83%9E%E3%83%83%E3%82%AD%E3%83%BC'
};
...
async function onePoster(){try{const h=await reqText(D.source);const u=metaImage(h,D.source);return await cachedImage(u,'one-event');}catch(_){return null;}}
async function heroContext(){
  ...
  if(KEY==='one'&&D.cardTba)return{a:{name:D.main.a,image:null},b:{name:D.main.b,image:null},poster:await onePoster()};
...
function bg(a,b){const c=new DrawContext();c.size=new Size(720,338);c.opaque=true;c.respectScreenScale=false;c.setFillColor(new Color('#040506'));c.fillRect(new Rect(0,0,720,338));if(a)c.drawImageInRect(a,imageRect(a,-5,360,'left'));if(b)c.drawImageInRect(b,imageRect(b,365,360,'right'));const shade=KEY==='rizin'?.78:.73;c.setFillColor(new Color('#000000',shade));c.fillRect(new Rect(0,0,720,338));c.setFillColor(new Color('#000000',.17));c.fillRect(new Rect(0,155,720,183));c.setFillColor(new Color('#000000',.15));c.fillRect(new Rect(0,220,720,118));c.setFillColor(new Color(S.accent,.022));c.fillRect(new Rect(352,0,16,338));return c.getImage();}
function posterBg(image){const c=new DrawContext();c.size=new Size(720,338);c.opaque=true;c.respectScreenScale=false;c.setFillColor(new Color('#050609'));c.fillRect(new Rect(0,0,720,338));if(image&&image.size){const iw=image.size.width,ih=image.size.height,scale=Math.max(720/iw,338/ih),dw=iw*scale,dh=ih*scale;c.drawImageInRect(image,new Rect((720-dw)/2,(338-dh)/2,dw,dh));}c.setFillColor(new Color('#000000',.78));c.fillRect(new Rect(0,0,720,338));c.setFillColor(new Color(S.accent,.035));c.fillRect(new Rect(0,0,720,338));return c.getImage();}
...
function dateText(d){if(D.timeTba)return'9/12 ・ 時刻未定';const f=new DateFormatter();f.locale='ja_JP';f.timeZone='Asia/Tokyo';f.dateFormat="M/d (E) HH:mm 'JST'";return f.string(new Date(d));}
...
if(KEY==='one'&&D.cardTba){
  w.addSpacer(22);
  const center=w.addStack();center.layoutVertically();const title=tx(center,'対戦カード発表待ち',13.5,new Color(C.text),'black',1);title.centerAlignText();center.addSpacer(4);const sub=tx(center,'ONE Friday Fights 168',8,new Color('#D8C86A'),'semibold');sub.centerAlignText();w.addSpacer(10);divider(w);w.addSpacer(6);const foot=w.addStack();foot.addSpacer();tx(foot,'公式更新を自動反映',7,new Color(C.muted),'semibold');foot.addSpacer();
}else{
  w.addSpacer(18);
  const main=w.addStack();main.centerAlignContent();const aBox=main.addStack();aBox.layoutVertically();aBox.size=new Size(136,0);tx(aBox,ctx.a.name,11.7,new Color(C.text),'black',2);main.addSpacer();const vsBox=main.addStack();vsBox.size=new Size(34,0);vsBox.centerAlignContent();const v=tx(vsBox,'VS',13.5,new Color(S.accent),'black');v.centerAlignText();main.addSpacer();const bBox=main.addStack();bBox.layoutVertically();bBox.size=new Size(136,0);const bn=tx(bBox,ctx.b.name,11.7,new Color(C.text),'black',2);bn.rightAlignText();w.addSpacer(3);const dv=tx(w,division(D.main.context),7.3,new Color('#BBC2CB'),'semibold');dv.centerAlignText();w.addSpacer(5);divider(w);w.addSpacer(4);
